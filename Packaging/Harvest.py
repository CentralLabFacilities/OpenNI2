...
class Harvest:
    def __init__(self, rootDir, outDir, arch, prefix='/usr'):
    ...
    def copySample(self, samplesDir, name, isLibrary=False, isGL=False, isJava=False):
        ...
        for root, dirs, files in os.walk(sampleSourceDir):
            ...
            # print dst
            for file in files:
                if (isJava and file.endswith('.java')) or (
                    not isJava and (file.endswith('.h') or file.endswith('.cpp'))):
        ...
        if self.osName == 'Windows':
            if isJava:
                ...
                shutil.copy(os.path.join(self.rootDir, 'ThirdParty', 'PSCommon', 'BuildSystem', 'BuildJavaWindows.py'),
                            sampleTargetDir)
                # fix call
                ...
                for group in doc.getElementsByTagName("ItemDefinitionGroup"):
                    ...
                    incDirs = \
                    group.getElementsByTagName('ClCompile')[0].getElementsByTagName('AdditionalIncludeDirectories')[0]
                    ...
                    libDirs = \
                    group.getElementsByTagName('Link')[0].getElementsByTagName('AdditionalLibraryDirectories')[0]
            ...
            shutil.copy(os.path.join(rootDir, 'ThirdParty', 'PSCommon', 'BuildSystem', 'CommonDefs.mak'),
                        sampleTargetDir)
            shutil.copy(os.path.join(rootDir, 'ThirdParty', 'PSCommon', 'BuildSystem', 'CommonTargets.mak'),
                        sampleTargetDir)
            ...
            if isJava:
                shutil.copy(os.path.join(rootDir, 'ThirdParty', 'PSCommon', 'BuildSystem', 'CommonJavaMakefile'),
                            sampleTargetDir)
            else:
                shutil.copy(os.path.join(rootDir, 'ThirdParty', 'PSCommon', 'BuildSystem', 'CommonCppMakefile'),
                            sampleTargetDir)
...
'''
        ...
        if isJava:
            ...
            # copy jar
            shutil.copy(os.path.join(self.binDir, 'org.openni.Samples.' + splitName[0] + '.jar'),
                        os.path.join(samplesDir, 'Bin'))
            # and script
            if not isLibrary:
                if self.osName == 'Windows':
                    shutil.copy(os.path.join(self.binDir, 'org.openni.Samples.' + splitName[0] + '.bat'),
                                os.path.join(samplesDir, 'Bin'))
                else:
                    shutil.copy(os.path.join(self.binDir, 'org.openni.Samples.' + splitName[0]),
                                os.path.join(samplesDir, 'Bin'))
        elif isLibrary:
    ...
    def run(self):
        ...
        try:
            ...
            print "WARNING: Could not copy Examples or Tools or Docs ---> %s" % e
        ...
        if self.osName == 'Windows':
            # Driver
            shutil.copytree(os.path.join(rootDir, 'ThirdParty', 'PSCommon', 'XnLib', 'Driver', 'Win32', 'Bin'),
                            os.path.join(self.outDir, 'Driver'))
            ...
            to_replace = 's/usr/%s/' % self.prefix
...
if len(sys.argv) < 3:
    print 'Usage: ' + sys.argv[0] + ' <OutDir> <x86|x64|Arm> <prefix>'
    exit(1)

rootDir = os.path.abspath(os.path.join(os.path.dirname(sys.argv[0]), '..'))
harvest = Harvest(rootDir, sys.argv[1], sys.argv[2], sys.argv[3])
harvest.run()
